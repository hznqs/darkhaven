...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  try {
    const payment = await prisma.$transaction(async (tx) => {
      ...
      if (current.status === "CONFIRMED") throw new Error("PAYMENT_CONFIRMED");
      if (current.status === "REFUNDED" || current.status === "CANCELED") throw new Error("PAYMENT_CLOSED");

      const updated = await tx.payment.update({
        where: { id },
        data: { status: "CANCELED", reason: parsed.data.reason },
    ...
    await writeAuditLogSafe({
      ...
      entity: "Payment",
      entityId: id,
      metadata: { canceled: true, reason: parsed.data.reason }
    });
    ...
    if (error instanceof Error) {
      ...
      if (error.message === "PAYMENT_CONFIRMED") return NextResponse.json({ error: "Pagamento confirmado deve ser estornado, não cancelado." }, { status: 409 });
      if (error.message === "PAYMENT_CLOSED") return NextResponse.json({ error: "Pagamento encerrado não pode ser cancelado." }, { status: 409 });
    }

    warnInDevelopment("Payment cancellation failed", error);
    return safeErrorResponse("Não foi possível cancelar o pagamento.");
  }
}
